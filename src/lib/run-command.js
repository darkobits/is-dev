 ...
 *
 * @param  {boolean} invert - Whether to invert the result of isDev().
 */
export default async function runCommand(invert) {
  const [,, command, ...args] = process.argv;

  // Provide the CWD here (which will be the path to the package.json where
  // if-dev / if-not-dev was executed) rather than using the default, which
  // would point back to this file.
